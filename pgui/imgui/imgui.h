...
struct ImVec2
{
	...
	const ImVec2 operator+(const ImVec2& lv)
	{
		return ImVec2(lv.x + x, lv.y + y);
	}
	ImVec2 operator-(const ImVec2& lv)
	{
		return ImVec2(x - lv.x, y - lv.y);
	}
	const ImVec2 operator*(const ImVec2& lv)
	{
		return ImVec2(lv.x * x, lv.y * y);
...
namespace ImGui
{
	...
	IMGUI_API int          Checkbox(const char* label, bool* v);
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_ButtonedChild = 1 << 29,
	// [Internal]
...
struct ImGuiListClipper
{
	...
	{
		memset(this, 0, sizeof(*this)); ItemsCount = -1; Begin(items_count, items_height);
	} // [removed in 1.79]
#endif
};
...
{
	ImGuiKeyModFlags_None = ImGuiModFlags_None, ImGuiKeyModFlags_Ctrl = ImGuiModFlags_Ctrl, ImGuiKeyModFlags_Shift = ImGuiModFlags_Shift, ImGuiKeyModFlags_Alt = ImGuiModFlags_Alt, ImGuiKeyModFlags_Super = ImGuiModFlags_Super
};

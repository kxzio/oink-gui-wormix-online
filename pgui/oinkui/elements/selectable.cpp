...
bool selectable_ex(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	int alpha = g_ui.process_animation(std::string(label + std::string(GetCurrentWindow( )->Name)).c_str( ), "alpha", hovered && (!selected), 200, 10.f, e_animation_type::animation_dynamic);
	int alpha_selected = g_ui.process_animation(std::string(label + std::string(GetCurrentWindow( )->Name)).c_str( ), "alpha_selected", selected, 255, 10.f, e_animation_type::animation_dynamic);
